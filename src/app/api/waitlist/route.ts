...
export async function POST(req: Request) {
  ...
  const payload = {
    email,
    source: "stacker-waitlist",
    ip,
    userAgent: req.headers.get("user-agent") || "",
    ts: new Date().toISOString(),
  };

  // Optional webhook: set STACKER_WAITLIST_WEBHOOK in Vercel env to pipe signups to
  // Slack, Discord, Zapier, n8n, etc. Failing webhook does not fail the request.
  const webhook = process.env.STACKER_WAITLIST_WEBHOOK;
  if (webhook) {
    try {
      await fetch(webhook, {
        method: "POST",
        ...
        body: JSON.stringify({
          text: `New /stacker waitlist: ${email}`,
          ...payload,
        }),
      });
    } catch {
  ...
  console.log("[stacker.waitlist]", JSON.stringify(payload));
