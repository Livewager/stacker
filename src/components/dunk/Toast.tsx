  ...
  return (
    ...
    >
      <div className="flex items-start gap-3">
        ...
        <button
          onClick={onDismiss}
          // Escape-on-focused-toast dismisses only this one (POLISH-
          // 270). Mirrors the Esc-closes-topmost-overlay pattern
          // POLISH-125 established for modals; here the "topmost"
          // anchor is the focused toast rather than a single global
          // overlay. stopPropagation so Esc doesn't also fire a
          // document-level handler that might trigger another
          // action (the global palette/modal ladder lives on
          // document, not on toast focus). For a burst of 5 errors
          // this turns "Tab Enter Tab Enter …" into "Esc Esc Esc" —
          // keyboard users can clear the stack without leaving the
          // home-row. Mouse users are unaffected (hover-pause +
          // × button still work).
          onKeyDown={(e) => {
            if (e.key === "Escape") {
              e.stopPropagation();
